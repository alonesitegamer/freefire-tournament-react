...
export default async function handler(req, res) {
  try {
    ...
    if (!ipAllowed || !emailAllowed) return json(res, 429, { error: "Too many OTP requests. Please try again later." });
    ...
    const transporter = nodemailer.createTransport({ service: "gmail", auth: { user: process.env.OTP_EMAIL, pass: process.env.OTP_PASS } });
