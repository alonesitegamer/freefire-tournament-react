...
function cleanString(value, field, max = MAX_STRING) {
  ...
  if (result.length > max) { const error = new Error(`${field} is too long`); error.status = 400; throw error; }
  return result;
}
function integer(value, field, min, max) {
  const result = Number(value);
  if (!Number.isInteger(result) || result < min || result > max) { const error = new Error(`${field} must be an integer between ${min} and ${max}`); error.status = 400; throw error; }
  return result;
}
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "DELETE") {
      ...
      const batch = db.batch();
      batch.delete(ref);
      batch.delete(db.collection("matchSecrets").doc(id));
      await batch.commit();
