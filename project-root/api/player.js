...
async function initProfile(uid, body) {
  ...
  const referral = String(body.referral || "").trim().toUpperCase().slice(0, 32);
  ...
  return db.runTransaction(async (tx) => {
    ...
    let referrerId = null;
    if (referral) {
      const referralQuery = db.collection("users").where("referralCode", "==", referral).limit(2);
      const matches = await tx.get(referralQuery);
      if (matches.size === 1 && matches.docs[0].id !== uid) referrerId = matches.docs[0].id;
    }
    ...
    tx.create(db.collection("economyLedger").doc(ledgerId("welcome", uid)), {
    ...
    return { status: 201, created: true, profile: data };
  });
}

async function updateProfile(uid, body) {
  const admin = getAdmin(); const db = admin.firestore(); const ref = db.collection("users").doc(uid); const patch = {};
  ...
  if (body.avatar !== undefined) {
    ...
    if (!avatar.startsWith("/avatars/") || avatar.includes("..")) { const error = new Error("Invalid avatar"); error.status = 400; throw error; }
    patch.avatar = avatar;
  }
...
async function updateStats(uid, body) {
  ...
  const idempotencyKey = stringField(body.idempotencyKey, "idempotencyKey", { min: 8, max: 128 });
  const admin = getAdmin(); const db = admin.firestore();
  const ledgerRef = db.collection("economyLedger").doc(ledgerId("stats", uid, idempotencyKey)); const userRef = db.collection("users").doc(uid);
  return db.runTransaction(async (tx) => {
    if ((await tx.get(ledgerRef)).exists) return { status: 200, existing: true };
    const snap = await tx.get(userRef); if (!snap.exists) return { status: 404, error: "Profile not found" };
    const stats = snap.data().stats || {};
    const next = { matchesPlayed: Number(stats.matchesPlayed || 0) + matches, totalKills: Number(stats.totalKills || 0) + kills, booyahs: Number(stats.booyahs || 0) + booyah, coinsEarned: Number(stats.coinsEarned || 0) };
...
export default async function handler(req, res) {
  try {
    method(req, "POST");
    const { user } = await requireUserWithAppCheck(req); const body = requireBody(req);
