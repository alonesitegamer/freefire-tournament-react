...
function idempotencyId(prefix, uid, key) {
  return `${prefix}_${uid}_${crypto.createHash("sha256").update(String(key || crypto.randomUUID())).digest("hex")}`;
}
...
async function dailyReward(uid) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const last = data.lastDaily?.toMillis?.() ?? 0;
    if (last && Date.now() - last < 24 * 60 * 60 * 1000) {
      return { status: 409, error: "Daily reward already claimed" };
    }

    const coins = Number(data.coins || 0) + 1;
    const xp = Number(data.xp || 0) + 10;
    tx.update(ref, {
      coins,
      xp,
      level: xpToLevel(xp),
    ...
    return { status: 200, coins, xp, level: xpToLevel(xp) };
  });
}
...
async function joinMatch(uid, body) {
  ...
  const ledgerRef = db.collection("economyLedger").doc(`join_${uid}_${matchId}`);

  return db.runTransaction(async (tx) => {
    const [userSnap, matchSnap, ledgerSnap] = await Promise.all([
      tx.get(userRef),
      tx.get(matchRef),
      tx.get(ledgerRef),
    ]);
    ...
    tx.update(userRef, {
      ...
      played: Number(user.played || 0) + 1,
    });
    tx.update(matchRef, { playersJoined: [...players, player] });
    tx.set(ledgerRef, {
      type: "match_join",
      uid,
      matchId,
...
async function createTopup(uid, body) {
  ...
  if (upiId.length < 3 || upiId.length > 100) return { status: 400, error: "Invalid payer UPI ID" };

  const admin = getAdmin();
  const db = admin.firestore();
  const requestRef = db.collection("topupRequests").doc(idempotencyId("topup", uid, key));
  const userSnap = await db.collection("users").doc(uid).get();
  if (!userSnap.exists) return { status: 404, error: "Profile not found" };
  const existing = await requestRef.get();
  if (existing.exists) return { status: 200, requestId: existing.id, existing: true };

  const email = userSnap.data().email || "";
  const coins = amount * TOPUP_RATE;
  await requestRef.create({ uid, userId: uid, email, amount, coins, upiId, status: "pending", createdAt: admin.firestore.FieldValue.serverTimestamp() });
...
async function createWithdrawal(uid, body) {
  ...
  const type = String(body?.type || "UPI");
  if (!["UPI", "Google Play", "Amazon"].includes(type)) return { status: 400, error: "Invalid withdrawal method" };
  const upiId = String(body?.upiId || "").trim();
  const email = String(body?.email || "").trim();
  if (type === "UPI" && (upiId.length < 3 || upiId.length > 100)) return { status: 400, error: "A valid UPI ID is required" };

  const reservedCoins = amount * TOPUP_RATE;
  const admin = getAdmin();
  const db = admin.firestore();
  const userRef = db.collection("users").doc(uid);
  const requestRef = db.collection("withdrawRequests").doc(idempotencyId("withdraw", uid, key));
...
async function adminTopup(uid, body) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (!requestSnap.exists) return { status: 404, error: "Top-up request not found" };
    const request = requestSnap.data();
    if (request.status !== "pending") return { status: 200, statusAlreadySet: true, status: request.status };

    const userRef = db.collection("users").doc(String(request.userId || uid));
    if (approve) {
      const userSnap = await tx.get(userRef);
      if (!userSnap.exists) return { status: 404, error: "User not found" };
      const user = userSnap.data();
      const coins = Number(user.coins || 0);
      const credit = Number(request.coins || 0);
      if (!Number.isInteger(credit) || credit <= 0) return { status: 500, error: "Invalid top-up credit" };
      tx.update(userRef, { coins: coins + credit });
      tx.set(db.collection("economyLedger").doc(`topup_${requestRef.id}`), {
        type: "topup_approved", requestId, uid: request.userId, amount: credit,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
      });
...
async function adminWithdrawal(uid, body) {
  ...
  return db.runTransaction(async (tx) => {
    const requestSnap = await tx.get(requestRef);
    if (!requestSnap.exists) return { status: 404, error: "Withdrawal request not found" };
    const request = requestSnap.data();
    if (request.status !== "pending") return { status: 200, statusAlreadySet: true, status: request.status };

    if (approve) {
      tx.update(requestRef, { status: "approved", processedAt: admin.firestore.FieldValue.serverTimestamp(), processedBy: uid });
      tx.set(db.collection("economyLedger").doc(`withdraw_${requestRef.id}`), {
        type: "withdraw_approved", requestId, uid: request.userId, amount: Number(request.reservedCoins || 0),
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
      });
      return { status: 200, approved: true };
    }
    ...
    if (!userSnap.exists) return { status: 404, error: "User not found" };
    const user = userSnap.data();
    tx.update(userRef, { coins: Number(user.coins || 0) + Number(request.reservedCoins || 0) });
    tx.update(requestRef, { status: "rejected", processedAt: admin.firestore.FieldValue.serverTimestamp(), processedBy: uid });
    tx.set(db.collection("economyLedger").doc(`withdraw_rejected_${requestRef.id}`), {
      type: "withdraw_rejected_refund", requestId, uid: request.userId, amount: Number(request.reservedCoins || 0),
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
    });
    return { status: 200, rejected: true };
  });
...
export default async function handler(req, res) {
  try {
    ...
    if (action.startsWith("admin_")) {
      ...
      const result = action === "admin_topup"
        ? await adminTopup(adminUser.uid, req.body)
        : action === "admin_withdrawal"
          ? await adminWithdrawal(adminUser.uid, req.body)
          : { status: 400, error: "Unknown admin action" };
