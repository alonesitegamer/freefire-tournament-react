...
function serialize(id, data) {
  return {
    id, email: data.email || "", username: data.username || "", displayName: data.displayName || "",
    avatar: data.avatar || "/avatars/default.jpg", bio: data.bio || "",
    coins: Number(data.coins || 0), xp: Number(data.xp || 0), level: Number(data.level || 1),
    wins: Number(data.wins || 0), played: Number(data.played || 0), referralCode: data.referralCode || "",
    lastDaily: data.lastDaily?.toDate?.()?.toISOString?.() || null,
    adsWatched: Number(data.adsWatched || 0), adsWatchedSinceReferral: Number(data.adsWatchedSinceReferral || 0),
    hasRedeemedReferral: Boolean(data.hasRedeemedReferral), referralRewardGiven: Boolean(data.referralRewardGiven), referrerId: data.referrerId || null,
  };
}
...
export default async function handler(req, res) {
  try {
    ...
    const patch = {};
    for (const field of PROFILE_FIELDS) {
      if (Object.prototype.hasOwnProperty.call(req.body || {}, field)) patch[field] = clean(req.body[field], field);
    }
    if (patch.avatar && !/^\/avatars\/[A-Za-z0-9._-]+\.(?:jpg|jpeg|png|webp)$/i.test(patch.avatar)) {
      const error = new Error("Invalid avatar path"); error.status = 400; throw error;
    }
