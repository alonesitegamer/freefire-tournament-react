...
export default function Dashboard({ user }) {
  ...
  function avatarRequiredLevel(index) {
    // first 6 avatars are free for everyone; after that require higher levels
    if (index <= 5) return 1;
    // gradually increase requirement
              ...
              {AVATARS.map((f, idx) => {
                ...
                return (
                  <div key={f} style={{ textAlign: "center" }}>
                    ...
                    <div style={{ marginTop: 8, color: "var(--muted)", fontSize: 12 }}>{f.replace(".jpg", "")}</div>
                  </div>
                );
              })}
            </div>
